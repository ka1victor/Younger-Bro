...
# @kaykyb's suggestion: load it once into a buffer, and then locate the buffer on the future times the function is called. 

from PIL import Image

hp_img_file = open('hp.png', 'rb')
hp_img = Image.open(hp_img_file)
# That could make the function faster, and decrease the time spent between action

def identify_status(): #'walkaround' or 'battle'
    global hp_img
    found = pyautogui.locateOnScreen(hp_img, confidence=0.8) 
